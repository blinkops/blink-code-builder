...
Table, read `workspace/tables/tables-schema.yaml` in the user's repo — one entry per table, with
...
Also: `row_count` in `workspace/tables/tables-schema.yaml` is Postgres's approximate live-tuple
