...
2. **Search the user's `workspace/workflows/` directory and any committed YAML for prior usage.** A working example in the repo beats any inferred shape. `grep -rn 'action: <full_name>' workspace/workflows/` is cheap.
