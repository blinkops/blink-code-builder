...
2. **Search the user's `workflows/` directory and any committed YAML for prior usage.** A working example in the repo beats any inferred shape. `grep -rn 'action: <full_name>' workflows/` is cheap.
...
- Column names aren't in the catalog. Confirm them with the user, or by grepping `workflows/` for a prior use of the same table, before writing `key:`.
