...
- `${CLAUDE_PLUGIN_DATA}/catalog/workspace_actions.tsv` — the workspace's callable actions (`action`, `name`, `kind`, `category`, `description`). A `kind=subflow` row is callable right now, and its `action` column is the exact `automations.<uuid>` string to put in the step. `kind=template` rows are the workspace's other own actions — same thing, callable as a step.
