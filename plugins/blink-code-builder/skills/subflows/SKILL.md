...
allowed-tools: Read, Grep, mcp__plugin_blink-code-builder_blink__list_connections, mcp__plugin_blink-code-builder_blink__fetch_automation, mcp__plugin_blink-code-builder_blink__validate_automation, mcp__plugin_blink-code-builder_blink__save_automation, mcp__plugin_blink-code-builder_blink__trigger_test_run, mcp__plugin_blink-code-builder_blink__get_run_log, mcp__plugin_blink-code-builder_blink__publish_automation
...
| Was published, then deactivated | no (the row disappears) | **No** — reactivate/publish it again first |
| Not `automation_type: on_demand` | no | **No** — only on-demand workflows can be subflows |
...
- `${CLAUDE_PLUGIN_DATA}/catalog/workspace_actions.tsv` — the workspace's callable actions (`action`, `name`, `kind`, `category`, `description`). A `kind=subflow` row is callable right now, and its `action` column is the exact `automations.<uuid>` string to put in the step. `kind=template` rows are the workspace's other own actions — same thing, callable as a step. Agents are not in this file — see `agents/agents-list.tsv` in the repo (generating-agent skill).
